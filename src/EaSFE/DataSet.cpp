...
#pragma omp parallel num_threads(ompthreadnums)
	...
	bool DataSet::write(const string& savepath, const string& other)
	{
		try {
			...
			for (int id = 0; id < m_numid; ++id)
			{
				...
				if (m_numid > 1)
					path += "/" + m_name + std::to_string(id) + "_afterfc" + endformat;
				else if (other == "")
					path += "_afterfc" + endformat, valpath += "_afterfc" + endformat, testpath += "_afterfc" + endformat;
	...
	PFeatureInfo DataSet::popFeature(bool onlydata)
	{
		...
		if (fsize <= 0)
		{
			LOG(ERROR) << "EaSFE::DataSet popFeature error: m_features is empty";
			return nullptr;
		}
